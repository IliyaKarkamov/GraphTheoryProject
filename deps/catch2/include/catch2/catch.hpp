...
struct Equals : StringHolder
{
    ...
    bool match(NSString* const& str) const override { return (str != nil || m_substr == nil) && [str isEqualToString:m_substr]; }
...
EnvironmentEstimate<FloatDuration<Clock>> estimate_clock_cost(FloatDuration<Clock> resolution)
{
    ...
    auto&& r =
        run_for_at_least<Clock>(std::chrono::duration_cast<ClockDuration<Clock>>(clock_cost_estimation_time), iters, time_clock);
...
Catch::Totals runTests(std::shared_ptr<Config> const& config)
{
    ...
    {
        bool matching =
            (!testSpec.hasFilters() && !testCase.isHidden()) || (testSpec.hasFilters() && matchTest(testCase, testSpec, *config));
...
auto operator<<(std::ostream& os, StringRef const& str) -> std::ostream& { return os.write(str.currentData(), str.size()); }
...
public:
    explicit Duration(double inNanoseconds, Unit units = Unit::Auto) : Duration(static_cast<uint64_t>(inNanoseconds), units) {}
...
{
    (*m_tablePrinter) << info.samples << ColumnBreak() << info.iterations << ColumnBreak() << Duration(info.estimatedDuration)
                      << ColumnBreak();
}
...
#define CATCH_CHECK_FALSE(...)                                                                                                   \
    INTERNAL_CATCH_TEST(                                                                                                         \
        "CATCH_CHECK_FALSE", Catch::ResultDisposition::ContinueOnFailure | Catch::ResultDisposition::FalseTest, __VA_ARGS__)
...
#define CATCH_FAIL_CHECK(...)                                                                                                    \
    INTERNAL_CATCH_MSG(                                                                                                          \
        "CATCH_FAIL_CHECK", Catch::ResultWas::ExplicitFailure, Catch::ResultDisposition::ContinueOnFailure, __VA_ARGS__)
...
#define CHECK_NOFAIL(...)                                                                                                        \
    INTERNAL_CATCH_TEST(                                                                                                         \
        "CHECK_NOFAIL", Catch::ResultDisposition::ContinueOnFailure | Catch::ResultDisposition::SuppressFail, __VA_ARGS__)
